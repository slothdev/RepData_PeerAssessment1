...
# Sets global options for R markdown
...
showing there are indeed `r length(unique(df$date))` unique days in the data set.  
...
```{r, histogram_totalsteps_estimate}
library(dplyr)
...
**mts**, the **mean** is **`r mean(mts$totalSteps)`** and the **median** is 
**`r median(mts$totalSteps)`**.  

```{r, mean_median_steps, cache=TRUE}
mean(mts$totalSteps)
...
**`r avgDaily[avgDaily$steps==max(avgDaily$steps),]$interval`**. If we
...
**`r nrow(df[!complete.cases(df),])`** rows.
...
+ We observe that there are **`r nrow(missing)`** NA rows from the
original dataset, and **`r length(unique(missing$date))`** days when they do occur.
Dividing the NA rows by the number of days with NA rows gives us exactly 
**`r nrow(missing)/length(unique(missing$date))`**, which is the exact number of
...
```{r, histogram_totalsteps_imputed}
